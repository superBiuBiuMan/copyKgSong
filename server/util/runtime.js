...
function applyCliOverrides(args) {
  ...
  if (parsed.port) {
    ...
    if (!Number.isNaN(port) && port > 0) {
      ...
      console.warn(
        `[cli] Invalid port value "${parsed.port}", fallback to default.`
      );
    }
  }
}
...
function resolveProxy() {
  const rawProxyEnv =
    typeof process.env.KUGOU_API_PROXY === "string"
      ...
      : undefined;
  const rawProxy =
    rawProxyEnv && rawProxyEnv.length > 0 ? rawProxyEnv : undefined;
  ...
  try {
    ...
    console.warn(
      `[proxy] Failed to parse proxy address "${rawProxy}": ${error.message}`
    );
    cachedProxy = null;
  }
